...
- `order` 表达学习依赖递进（先修概念在前），仅在同一 topic 内比较；新增笔记默认取该 topic 现有 max(order)+1；若新笔记是现有笔记的先修（更基础），则插入对应位置并将受影响的现有笔记 `order` 顺延重排（保持主题内唯一递增——`order` 只是排序键，改动不影响卡片与复习状态）。
...
1. 读取 `data/inbox/` 中指定文件。**PDF 优先调服务端接口提取文本层**：`curl -s "http://127.0.0.1:5574/api/materials/extract-text?name=<文件名>"`（纯 Go 实现随二进制分发，零安装；加密 PDF 自动尝试空密码解密）。返回 `text` 为空/近空（`chars` 很小）说明是**扫描件**：此时若本机有 poppler（`pdftoppm -png -r 150 文件 页前缀`）则转图后逐页视觉阅读，量大时改用本地 OCR 小模型（如腾讯 HunyuanOCR）；都没有就如实报告并中止。DOCX/EPUB 等用你可用的解析能力提取文本；无法解析时如实报告。**若参数是 http(s) URL**：用你自带的网页抓取能力（如 FetchURL）抓取并理解正文（避开导航/广告等噪声），将正文整理为 markdown 写入 `data/inbox/<kebab-标题>.md` 作为归档原件，随后按普通文件流程继续；抓取失败（登录墙、纯 JS 渲染、反爬）或你没有抓取能力时如实报告并中止，不许编造内容。
